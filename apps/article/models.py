...
class EnArticleType(models.Model):
...
class EnArticles(models.Model):
    ...
    title = models.TextField(verbose_name='文章标题',
                             help_text='文章标题')
    sub_title = models.TextField(verbose_name='文章副标题',
                                 help_text='文章副标题')

    summarize = models.TextField(verbose_name='文章概述',
                                 null=True,
                                 blank=True,
                                 help_text='文章概述')
    ...
    types = models.ForeignKey(EnArticleType,
                              related_name='enarticle',
    ...
    user = models.ForeignKey(MyUser, related_name='enarticle', on_delete=models.CASCADE)

    def __str__(self):
        return self.title

    class Meta:
        verbose_name = '[英文En]新闻/文章'
        verbose_name_plural = verbose_name


class EnCertificate(models.Model):
    create_time = models.DateTimeField(auto_now_add=True)
    ...
    cert_type = models.CharField(max_length=12, choices=CERT_TYPE_CHOICE, verbose_name='证书类型', help_text='证书类型')

    def __str__(self):
        return self.title

    class Meta:
        verbose_name = '[英文En]技术/荣誉证书'
        verbose_name_plural = verbose_name
